...
contract Dungeons is IDungeons, ERC721Enumerable, ReentrancyGuard, Ownable {
   ...
   *       e.g. claim(43)
   */
  function claim(uint256 tokenId) public payable override nonReentrant {
   ...
   */
  function claimMany(uint256[] memory tokenArray) public payable override nonReentrant {
   ...
   *        e.g. ownerClaim(7779)
   */
  function ownerClaim(uint256 tokenId) public payable override nonReentrant onlyOwner {
   ...
   */
  function openClaim() public override nonReentrant onlyOwner {
   ...
   */
  function withdraw(address payable recipient, uint256 amount) public override nonReentrant onlyOwner {
  ...
  /* Read Functions */
  function tokenURI(uint256 tokenId) public view override(ERC721, IDungeons) returns (string memory) {
   ...
   */
  function getEntities(uint256 tokenId)
    public
    view
    override
    returns (
  ...
  constructor(
    // ILoot _lootContract,
    IDungeonsRender _render,
    IDungeonsGenerator _generator,
    IDungeonsSeeder _seeder
  ) ERC721('Crypts and Caverns', 'CAVERNS') Ownable() {
