 ...
 */
library StringUtil {
  function titleCase(string memory _base) internal pure returns (string[] memory) {
    ...
    for (uint8 i = 0; i < components.length; ++i) {
      ...
      string memory remainingChars = _substring(components[i], int256(StringUtil.length(components[i]) - 1), 1);
