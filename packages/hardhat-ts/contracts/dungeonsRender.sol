...
/// https://etherscan.io/address/0xf701f9cd49216fa6111ba4c8d41227178592e9b4#code
...
contract dungeonsRender {
  ...
  function draw(
    IDungeons.Dungeon memory dungeon,
    uint8[] memory x,
    uint8[] memory y,
    uint8[] memory entityData
    ...
    for (uint256 y = 0; y < dungeon.size; y++) {
      ...
      for (uint256 x = 0; x < dungeon.size; x++) {
        if (getBit(helper.layout, helper.counter) == 1 && helper.counter > 0 && getBit(helper.layout, helper.counter - 1) == 0) {
          ...
          rowParts = drawTile(
            rowParts,
            helper.start + (helper.lastStart % dungeon.size) * helper.pixel,
            helper.start + (helper.lastStart / dungeon.size) * helper.pixel,
            (helper.counter - helper.lastStart) * helper.pixel,
            helper.pixel,
            colors[dungeon.environment * 4 + 1]
          );
  ...
  function drawEntities(
    ...
    for (uint256 i = 0; i < entityData.length; i++) {
      parts = drawTile(
        parts,
        helper.start + (x[i] % dungeon.size) * helper.pixel,
        helper.start + y[i] * helper.pixel,
        helper.pixel,
        helper.pixel,
        colors[dungeon.environment * 4 + 2 + entityData[i]]
      );
    }
    return parts;
  }
