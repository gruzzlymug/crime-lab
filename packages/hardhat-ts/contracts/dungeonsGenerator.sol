...
/// https://etherscan.io/txs?a=0x34734543e8a2505224a7b4ad42ac89ad88fef39a&p=2
    ...
    while (numRooms > 0) {
      ...
      if (rooms[0].x != 0) {
        ...
        for (uint256 i = 0; i < rooms.length - numRooms; i++) {
          ...
          if (
            rooms[i].x - 1 < current.x + current.width &&
            rooms[i].x + rooms[i].width + 1 > current.x &&
            rooms[i].y - 1 < current.x + current.height &&
            rooms[i].y + rooms[i].height > current.y
          ) {
